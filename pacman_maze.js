...
function generateSvg(data, theme = "dark") {
  ...
  //
  // TITLE + GHOST PNG SYMBOLS
  //
  svg_content += `
    <defs>
      <symbol id="ghost-blinky" viewBox="0 0 20 20">
        <image href="data:image/png;base64,<REDACTED>" width="20" height="20"/>
      </symbol>
  ...
  //
  const ghosts = [
    { id: "ghost-blinky", delay: 0.5 },
    { id: "ghost-pinky", delay: 1.0 },
    { id: "ghost-inky", delay: 1.5 },
    { id: "ghost-clyde", delay: 2.0 },
  ];
    ...
    svg_content += `
      <g>
          <use href="#${ghost.id}" x="-10" y="-10" width="20" height="20">
              <animateMotion path="${animPath}" begin="${ghost.delay}s" dur="${duration}s" repeatCount="indefinite" rotate="auto" />
          </use>
      </g>
    `;
  });
